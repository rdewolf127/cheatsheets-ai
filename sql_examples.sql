/* To run through this practice you'll want to download Postgres SQL to your laptop.  
For windows https://www.postgresql.org/download/windows/
For mac https://www.postgresql.org/download/macosx/

You can use all the defaults in the installer and uncheck the stackbuilder option at the end (you don't need it)
DON'T FORGET THE PASSWORD YOU USE DURING THE INSTALL!!!!  You WILL need that.

The installer will put something called pgAdmin4 on your system.  This is what you will use to run your SQL.  It should feel a bit like TOAD.

Open pgAdmin4 
Click on servers (you will get prompted for that password duing this)
Click on postgresSQL
Click on databases.  You will see a default database.  Ignore it.  Right click on databases to create a new one.
Click on the plus sign next to your new database 
Click on Schemas
Click on public.
You should see now an option for tables.  If you click on that plus there won't be anything there yet.  We're going to add tables to that new 
database.  
Go up to the menu bar and click on Tools and select Query Tool.  It'll open a query writer window and show up a the top the name of the DB you
just created.  This is where you are going to drop the SQL below*/

/*In case you hadn't noticed by now  :) , in SQL You can put anything you don't want to run and just be visible as a comment between /* */ 


/*These first two bits of sql will create and insert data into the table that the rest of the SQL will run on. they also serve to demonstrate
the syntax for the create table and insert/append data functions  :)  */
...
create table staff
 ...
 /*NOTE - This data came from a Linked In SQL tutorial*/
...
create table job_title_avg_salary as
select
	job_title,
...
update 
	staff
set 
	salary = (salary + (salary*.05)) /*give everyone a 5% increase :)*/
where 
	salary > 0;

/*deleting a value in a table*/
delete from 
	staff
where 
	salary >125000 /*fire everyone making more than 100K :)*/

/*basic select query*/
select 
	job title, /*these are the field names*/
	salary
from
	staff; /*this is the table name*/
...
group by 
	job_title
order by 
	job_title DESC; /*the default is ascending and you dont have to specify it but you do specify DESC for descending*/

/*create an alias for a derived field*/
select
	job_title,
...
/*do other math functions*/
select 
	job_title,
	max(salary) as highest_salary, min(salary) as lowest_salary, avg(salary), count(salary)
from
	staff
group by
	job_title;

/*format decimal places with the round function*/
select 
	job_title,
...
/*use cast function to change a field from one data type to another*/
select
	job_title,
	salary, /*this is the original salary field which is an integer*/
	cast(salary as varchar) as salary_varchar /*this creates a new field called salary_varchar with the salary as a character field vs. a integer*/
from
	staff;

/*use * selector*/
select *  /*this selects all fields on the table without calling each out individually*/
from
	staff;

/*use * select in count*/
select 
	count(*) /*this will count all rows in the table*/
from 
	staff;

/*use the distinct function*/
select distinct  /*this will give you back just one row with each unique job title*/
	job_title
from 
	staff;
...
/*join two tables*/
select 
	s.job_title, /*you add the alias for each table in front of the fields you want from that table*/
	s.salary,
	a.avg_salary
from 
	staff s /*the s here is just an alias you make up to distinguish one table from another*/
join 
	job_title_avg_salary a  /*join will only grab records where there is a match on both tables.  You can also left join or right join to keep all records from one of the tables regardless of whether there is a match*/
on 
...
look at an individuals salary next to the average for their role.  This could also have been managed in a subquery*/ 
...
select 
    ...
    s1.job_title,
    s1.salary,
    (select round(avg(salary)) from staff s2 where s2.department = s1.department)as dept_salary /*this selects the avg department salary for departments in the top level query*/
from
	staff s1;

/*some other subquery examples*/

/*subquery in from statement*/
select 
	s1.department,
    round(avg(s1.salary)) as avg_exec_salary
from
	(select 
 		department,
 		salary
 	from 
 		staff
 	where
 		salary >100000) s1 /*this selects only executives with salaries greater than 100,000*/
group by department;
...
where s1.salary = 
					(select
   						 max(s2.salary)
					from staff s2); /*this selects only the max salary amount*/

/*grouping sets*/
/*this query counts the number of employees in each grouping set (by department and then by job title and returns both sets of results)*/
select 
	department,
	job_title,
	count(*)
...
from 
	staff
group by 
	cube(department, job_title);  /*this would be what you might use for A/R/D/S rollups*/

/*limit to top N results based on a sort order*/
select 
	salary
from 
	staff
